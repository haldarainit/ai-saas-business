...
class CampaignStorage {
  ...
  async loadCampaignRecipient(campaignId, recipientIndex) {
    try {
      ...
      // IMPORTANT: Include userId so we can pass it to email logging
      const campaign = await Campaign.findOne(
        ...
        {
          userId: 1, // Required for email history logging
          subject: 1,
          template: 1,
          status: 1,
          sentCount: 1,
  ...
  async logEmailSent(emailData) {
    try {
      ...
      // Resolve userId in priority: provided -> instance -> campaign.owner
      ...
      if (!effectiveUserId && emailData.campaignId) {
        try {
          ...
          if (owningCampaign && owningCampaign.userId) {
            effectiveUserId = owningCampaign.userId;
          }
        } catch (_) {
          // fallback to null if lookup fails; we still proceed
        }
      }

      // CRITICAL: userId is required by the schema - fail early if we can't resolve it
      if (!effectiveUserId) {
      ...
      const update = {
        $set: {
          userId: effectiveUserId, // Always set userId since we validated it above
      ...
      if (error.code === 11000) {
        ...
        // Re-resolve userId for the update (same logic as above)
        ...
        if (!resolvedUserId) {
          console.error("❌ Cannot update existing log: userId could not be resolved");
          return { success: false, error: "userId is required but could not be resolved" };
        }
  ...
  async initializeCampaignLogs(campaignId, recipients, subject, userId = null) {
    try {
      await this.ensureConnection();

      console.log(`📋 Initializing ${recipients.length} email logs for campaign ${campaignId}`);

      // Resolve userId: explicit param -> instance -> campaign owner
      ...
      // CRITICAL: Don't initialize logs without a valid userId
      if (!effectiveUserId) {
      ...
      const bulkOps = recipients.map((recipient) => ({
        updateOne: {
          ...
          update: {
            $set: {
              // Always update userId to ensure it's set even on existing records
              userId: effectiveUserId,
            },
            $setOnInsert: {
              campaignId: campaignId,
  ...
  // NOTE: initializeCampaignLogs is defined above using bulkWrite with upsert
