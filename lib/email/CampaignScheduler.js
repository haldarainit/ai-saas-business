...
class CampaignScheduler {
  ...
  async processNextEmail() {
    try {
      ...
      // Send email
      console.log(
        `Sending email ${currentIndex + 1}/${recipients.length
        } to: ${recipientEmail}`
      );
    ...
    try {
      ...
      if (existingCampaignResult.success && existingCampaignResult.data) {
        ...
        if (existingCampaign.status === "completed") {
          console.log(
            "⚠️ Campaign is completed - keeping it as-is. No new campaigns or updates allowed."
          );
          return {
            success: true,
      ...
      // If a NEW csv file is uploaded (different from stored one) wipe NON-COMPLETED campaigns/logs
      if (csvData && csvData.totalRows) {
        ...
        if (
          ...
          console.log(
            "🧹 New CSV detected – clearing existing non-completed campaigns & logs before saving fresh data"
          );
          // Only delete non-completed campaigns
          await this.storage.deleteNonCompletedCampaigns();
