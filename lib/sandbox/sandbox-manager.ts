// Sandbox Manager - Singleton for managing sandbox instances
import { SandboxProvider } from '@/types/sandbox';
...
class SandboxManager {
   ...
   */
  getActiveSandboxInfo(): { sandboxId: string; url: string } | null {
    if (!this.activeSandboxId) return null;
    const sandbox = this.sandboxes.get(this.activeSandboxId);
    if (!sandbox) return null;
    const info = sandbox.provider.getSandboxInfo();
    if (!info) return null;
