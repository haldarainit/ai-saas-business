// AI Provider Resolution System
// Routes model requests to the correct provider (OpenAI, Anthropic, Groq, Google)
...
function getEnvDefaults(provider: ProviderName): { apiKey?: string; baseURL?: string } {
    ...
    switch (provider) {
        ...
        case 'google':
            return { apiKey: process.env.GEMINI_API_KEY || process.env.GOOGLE_API_KEY, baseURL: process.env.GEMINI_BASE_URL };
        default:
            return {};
    }
}
...
function getOrCreateClient(provider: ProviderName, apiKey?: string, baseURL?: string): ProviderClient {
    // Prefer direct provider keys for Google to avoid routing through AI Gateway
    const effective = isUsingAIGateway && provider !== 'google'
    ...
    switch (provider) {
        case 'openai':
            client = createOpenAI({
                apiKey: effective.apiKey || getEnvDefaults('openai').apiKey,
                baseURL: effective.baseURL ?? getEnvDefaults('openai').baseURL
            });
            break;
        case 'anthropic':
            client = createAnthropic({
                apiKey: effective.apiKey || getEnvDefaults('anthropic').apiKey,
                baseURL: effective.baseURL ?? getEnvDefaults('anthropic').baseURL
            });
            break;
        case 'groq':
            client = createGroq({
                apiKey: effective.apiKey || getEnvDefaults('groq').apiKey,
                baseURL: effective.baseURL ?? getEnvDefaults('groq').baseURL
            });
            break;
        case 'google':
            client = createGoogleGenerativeAI({
                apiKey: effective.apiKey || getEnvDefaults('google').apiKey,
                baseURL: effective.baseURL ?? getEnvDefaults('google').baseURL
            });
            break;
        default:
            client = createGroq({
