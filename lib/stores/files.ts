...
import { getEncoding, isBinary } from 'istextorbinary';
    ...
    async createFile(filePath: string, content: string | Uint8Array = '') {
      ...
      async deleteFile(filePath: string) {
        ...
        try {
          ...
          // Track deleted paths to prevent them from reappearing
          this.#deletedPaths.add(filePath);
          this.#saveDeletedPaths();
      ...
      async deleteFolder(folderPath: string) {
        ...
        try {
          ...
          // Also remove all files/folders within this folder from the store
          for (const [path, dirent] of Object.entries(this.files.get())) {
      ...
      #saveDeletedPaths() {
        try {
          ...
          logger.error('Failed to save deleted paths to localStorage', error);
        }
      }
    }
