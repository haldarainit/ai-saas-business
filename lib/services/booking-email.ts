...
interface ResolvedEmailSettings extends EmailSettings {
  emailUser: string;
  emailPassword: string;
  emailProvider: string;
}
...
function resolveEmailSettings(
  ...
  const emailPassword =
    emailSettings?.emailPassword || process.env.EMAIL_PASSWORD || "";
  ...
  if (!resolved.fromName && process.env.EMAIL_FROM_NAME) {
    resolved.fromName = process.env.EMAIL_FROM_NAME;
  }

  if (resolved.emailProvider === "custom") {
    if (!resolved.smtpHost && process.env.EMAIL_SMTP_HOST) {
      resolved.smtpHost = process.env.EMAIL_SMTP_HOST;
    }

    if (!resolved.smtpPort && process.env.EMAIL_SMTP_PORT) {
      const parsed = Number(process.env.EMAIL_SMTP_PORT);
      if (!Number.isNaN(parsed)) {
        resolved.smtpPort = parsed;
      }
    }

    if (
      typeof resolved.smtpSecure !== "boolean" &&
      process.env.EMAIL_SMTP_SECURE
    ) {
      resolved.smtpSecure = process.env.EMAIL_SMTP_SECURE === "true";
    }
  }
...
function createTransporter(
  ...
  const resolved = resolveEmailSettings(emailSettings);

  if (!resolved) {
    throw new Error(
      "Email settings not configured. Please add credentials in Settings or set EMAIL_USER/EMAIL_PASSWORD env vars.",
    );
  }

  const provider = resolved.emailProvider || "gmail";
  const providerConfig = EMAIL_PROVIDERS[provider] || EMAIL_PROVIDERS.custom;
  const useCustom = provider === "custom";

  const host =
    useCustom ? resolved.smtpHost || providerConfig.host : providerConfig.host;
  const port =
    useCustom ? resolved.smtpPort || providerConfig.port : providerConfig.port;
  const secure =
    useCustom ?
      typeof resolved.smtpSecure === "boolean" ?
        resolved.smtpSecure
  ...
  if (!host) {
    throw new Error(
      "SMTP host not configured. Please select a provider or configure EMAIL_SMTP_HOST.",
    );
  }
  ...
  return nodemailer.createTransport({
    ...
    secure: !!secure,
    auth: {
      user: resolved.emailUser,
      pass: resolved.emailPassword,
    },
  });
}
...
export async function sendReminderEmail(
  ...
  try {
    ...
    const timeText =
      hoursUntilMeeting === 1 ? "1 hour" : `${hoursUntilMeeting} hours`;
...
export async function sendBookingEmails(
  ...
  const emailSettings = resolveEmailSettings(host.emailSettings);
