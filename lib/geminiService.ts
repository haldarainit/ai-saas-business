...
// Available models in order of preference (aligned with utils/gemini.ts)
...
export async function getTrendingProducts(category: string): Promise<TrendingProductsResponse> {
    try {
        ...
        throw new Error(`Failed to get dynamic market data: ${err.message}`);
    }
}
...
IMPORTANT: Return ONLY raw JSON, no markdown code blocks.`;
        ...
        const parsedData = JSON.parse(jsonMatch[0]) as PresentationContent;
        ...
        if (parsedData.slides) {
            parsedData.slides = parsedData.slides.map((slide, index) => {
                ...
                if (slide.content && Array.isArray(slide.content)) {
                    slide.content = slide.content.map((item: unknown) => {
                        if (typeof item === 'string') {
                            ...
                            return obj.text || obj.description || obj.title || JSON.stringify(item);
                        }
                        return String(item);
                    });
                }
        ...
        const err = error as Error;
        console.error('❌ Gemini Enhanced Presentation API failed:', err.message);
        throw new Error(`Failed to generate presentation: ${err.message}`);
    }
}
