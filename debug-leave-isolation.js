...
async function debugLeaveRequest() {
  ...
  try {
    ...
    const { default: Leave } = await import('./lib/models/Leave.js');
    const { default: Employee } = await import('./lib/models/Employee.js');
    
    await dbConnect();
    
    // Find the leave request for Raj (EMP124)
    console.log('\n📋 Checking leave request for EMP124 (Raj)...');
    const leave = await Leave.findOne({ employeeId: 'EMP124' }).lean();
    ...
    const employee = await Employee.findOne({ employeeId: 'EMP124' }).lean();
    ...
    const allEmployees = await Employee.find({}).select('employeeId name userId').lean();
    ...
    const allLeaves = await Leave.find({}).select('employeeId employeeName userId leaveType status').lean();
