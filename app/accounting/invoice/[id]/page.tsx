...
export default function InvoicePage() {
    ...
    useEffect(() => {
        const fetchInvoice = async () => {
            try {
                ...
                if (inv) {
                    // Normalize data from DB structure to State structure
                    // The Mongoose model and this State might slightly differ in nesting
                    // We need to map them correctly.

                    const mappedData: InvoiceData = {
                        title: inv.title || defaultInvoiceData.title,
                        // Toggles
                        showBankDetails: inv.settings?.showBankDetails ?? defaultInvoiceData.showBankDetails,
                        showTerms: inv.settings?.showTerms ?? defaultInvoiceData.showTerms,
                        showJurisdiction: inv.settings?.showJurisdiction ?? defaultInvoiceData.showJurisdiction,
                        showDeclaration: inv.settings?.showDeclaration ?? defaultInvoiceData.showDeclaration,
                        showDiscount: inv.settings?.showDiscount ?? defaultInvoiceData.showDiscount,
                        showTaxColumns: inv.settings?.showTaxColumns ?? defaultInvoiceData.showTaxColumns,
                        showDueDate: inv.settings?.showDueDate ?? defaultInvoiceData.showDueDate,
                        showDeliveryDetails: inv.settings?.showDeliveryDetails ?? defaultInvoiceData.showDeliveryDetails,
                        showDispatchDetails: inv.settings?.showDispatchDetails ?? defaultInvoiceData.showDispatchDetails,
                        showRoundOff: inv.settings?.showRoundOff ?? defaultInvoiceData.showRoundOff,
                        showHSNSAC: inv.settings?.showHSNSAC ?? defaultInvoiceData.showHSNSAC,
                        currency: inv.settings?.currency ?? defaultInvoiceData.currency,
                        currencySymbol: inv.settings?.currencySymbol ?? defaultInvoiceData.currencySymbol,
                        taxType: inv.settings?.taxType ?? defaultInvoiceData.taxType,
                        gstDisplayMode: inv.settings?.gstDisplayMode ?? defaultInvoiceData.gstDisplayMode,

                        // Company
                        companyName: inv.companyDetails?.name || "",
                        companyAddress: inv.companyDetails?.address || "",
                        companyCity: inv.companyDetails?.city || "",
                        companyState: inv.companyDetails?.state || "",
                        companyPincode: inv.companyDetails?.pincode || "",
                        companyEmail: inv.companyDetails?.email || "",
                        ...
                        clientAddress: inv.clientDetails?.address || "",
                        clientCity: inv.clientDetails?.city || "",
                        clientState: inv.clientDetails?.state || "",
                        clientPincode: inv.clientDetails?.pincode || "",
                        clientEmail: inv.clientDetails?.email || "",
                        clientPhone: inv.clientDetails?.phone || "",
                        clientGSTIN: inv.clientDetails?.gstin || "",
                        paymentMode: inv.clientDetails?.paymentMode || defaultInvoiceData.paymentMode,

                        // Main
                        invoiceNumber: inv.invoiceNumber || "",
                        invoiceDate: inv.invoiceDate ? new Date(inv.invoiceDate).toISOString().split('T')[0] : "",
                        dueDate: inv.dueDate ? new Date(inv.dueDate).toISOString().split('T')[0] : "",
                        poDate: inv.poDate ? new Date(inv.poDate).toISOString().split('T')[0] : "",
                        poNumber: inv.poNumber || "",

                        // Ship To
                        shipToName: inv.shipToDetails?.name || "",
                        shipToAddress: inv.shipToDetails?.address || "",
                        shipToCity: inv.shipToDetails?.city || "",
                        shipToState: inv.shipToDetails?.state || "",
                        shipToPincode: inv.shipToDetails?.pincode || "",
                        sameAsBillTo: inv.shipToDetails?.sameAsBillTo ?? true,

                        // Order/Dispatch
                        placeOfSupply: inv.orderDetails?.placeOfSupply || "",
                        reverseCharge: inv.orderDetails?.reverseCharge || "No",
                        hsnsac: inv.orderDetails?.hsnsac || "",
                        deliveryNote: inv.orderDetails?.deliveryNote || "",
                        deliveryNoteDate: inv.orderDetails?.deliveryNoteDate || "",
                        referenceNo: inv.orderDetails?.referenceNo || "",
                        referenceDate: inv.orderDetails?.referenceDate || "",
                        otherReferences: inv.orderDetails?.otherReferences || "",
                        dispatchDocNo: inv.orderDetails?.dispatchDocNo || "",
                        dispatchedThrough: inv.orderDetails?.dispatchedThrough || "",
                        destination: inv.orderDetails?.destination || "",
                        termsOfDelivery: inv.orderDetails?.termsOfDelivery || "",

                        // Items
                        items: inv.items.length > 0 ? inv.items : defaultInvoiceData.items,

                        // Financials
                        shippingCharges: inv.financials?.shippingCharges || 0,
                        otherCharges: inv.financials?.otherCharges || 0,
                        ...
                        upiId: inv.bankDetails?.upiId || "",

                        // Terms
                        notes: inv.terms?.notes || "",
                        termsConditions: inv.terms?.termsConditions || defaultInvoiceData.termsConditions,
                        jurisdictionText: inv.terms?.jurisdictionText || defaultInvoiceData.jurisdictionText,
                        authorizedSignatory: inv.terms?.authorizedSignatory || "",
                        declarationText: inv.terms?.declarationText || defaultInvoiceData.declarationText,
                    };
    ...
    // Auto-Save Logic
    const debouncedInvoiceData = useDebounce(invoiceData, 2000);
    ...
    useEffect(() => {
        ...
        const saveInvoice = async () => {
            setIsSaving(true);
            try {
                // Map flat state back to nested DB structure
                const dbPayload = {
                    title: debouncedInvoiceData.title,
                    invoiceNumber: debouncedInvoiceData.invoiceNumber,
                    invoiceDate: debouncedInvoiceData.invoiceDate,
                    dueDate: debouncedInvoiceData.dueDate,
                    poDate: debouncedInvoiceData.poDate,
                    poNumber: debouncedInvoiceData.poNumber,

                    settings: {
                        showBankDetails: debouncedInvoiceData.showBankDetails,
                        showTerms: debouncedInvoiceData.showTerms,
                        showJurisdiction: debouncedInvoiceData.showJurisdiction,
                        showDeclaration: debouncedInvoiceData.showDeclaration,
                        showDiscount: debouncedInvoiceData.showDiscount,
                        showTaxColumns: debouncedInvoiceData.showTaxColumns,
                        showDueDate: debouncedInvoiceData.showDueDate,
                        showDeliveryDetails: debouncedInvoiceData.showDeliveryDetails,
                        showDispatchDetails: debouncedInvoiceData.showDispatchDetails,
                        showRoundOff: debouncedInvoiceData.showRoundOff,
                        showHSNSAC: debouncedInvoiceData.showHSNSAC,
                        currency: debouncedInvoiceData.currency,
                        currencySymbol: debouncedInvoiceData.currencySymbol,
                        taxType: debouncedInvoiceData.taxType,
                        gstDisplayMode: debouncedInvoiceData.gstDisplayMode
                    },
                    companyDetails: {
                        name: debouncedInvoiceData.companyName,
                        address: debouncedInvoiceData.companyAddress,
                        city: debouncedInvoiceData.companyCity,
                        state: debouncedInvoiceData.companyState,
                        pincode: debouncedInvoiceData.companyPincode,
                    ...
                    clientDetails: {
                        ...
                        address: debouncedInvoiceData.clientAddress,
                        city: debouncedInvoiceData.clientCity,
                        state: debouncedInvoiceData.clientState,
                        pincode: debouncedInvoiceData.clientPincode,
                        email: debouncedInvoiceData.clientEmail,
                        phone: debouncedInvoiceData.clientPhone,
                        gstin: debouncedInvoiceData.clientGSTIN,
                        paymentMode: debouncedInvoiceData.paymentMode
                    },
                    shipToDetails: {
                        name: debouncedInvoiceData.shipToName,
                        address: debouncedInvoiceData.shipToAddress,
                        city: debouncedInvoiceData.shipToCity,
                        state: debouncedInvoiceData.shipToState,
                        pincode: debouncedInvoiceData.shipToPincode,
                        sameAsBillTo: debouncedInvoiceData.sameAsBillTo
                    },
                    orderDetails: {
                        placeOfSupply: debouncedInvoiceData.placeOfSupply,
                        reverseCharge: debouncedInvoiceData.reverseCharge,
                        hsnsac: debouncedInvoiceData.hsnsac,
                        deliveryNote: debouncedInvoiceData.deliveryNote,
                        deliveryNoteDate: debouncedInvoiceData.deliveryNoteDate,
                        referenceNo: debouncedInvoiceData.referenceNo,
                        referenceDate: debouncedInvoiceData.referenceDate,
                        otherReferences: debouncedInvoiceData.otherReferences,
                        dispatchDocNo: debouncedInvoiceData.dispatchDocNo,
                        dispatchedThrough: debouncedInvoiceData.dispatchedThrough,
                        destination: debouncedInvoiceData.destination,
                        termsOfDelivery: debouncedInvoiceData.termsOfDelivery
                    },
                    ...
                    bankDetails: {
                        ...
                        accountNumber: debouncedInvoiceData.accountNumber,
                        ifscCode: debouncedInvoiceData.ifscCode,
                        branch: debouncedInvoiceData.branch,
                        upiId: debouncedInvoiceData.upiId
                    },
                    terms: {
                        notes: debouncedInvoiceData.notes,
                        termsConditions: debouncedInvoiceData.termsConditions,
                        jurisdictionText: debouncedInvoiceData.jurisdictionText,
                        authorizedSignatory: debouncedInvoiceData.authorizedSignatory,
                        declarationText: debouncedInvoiceData.declarationText
                    }
                };

                await fetch(`/api/invoice/${params.id}`, {
                    method: 'PUT',
        ...
        saveInvoice();
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [debouncedInvoiceData, params.id, isLoading]);
        ...
        <div className="flex flex-col min-h-screen bg-background text-foreground transition-colors duration-300" suppressHydrationWarning>
            ...
            <div className="sticky top-0 z-30 bg-background/95 backdrop-blur border-b shadow-sm px-4 py-3 flex items-center justify-between">
                <div className="flex items-center gap-4">
                    <Link href="/accounting/invoice" className="text-muted-foreground hover:text-foreground">
                        <ArrowLeft className="w-5 h-5" />
                    </Link>
                    <div className="flex flex-col">
                        <h1 className="text-xl font-semibold">
                            {isLoading ? "Loading..." : invoiceData.invoiceNumber || "New Invoice"}
                        </h1>
