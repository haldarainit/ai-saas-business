...
interface RecipientsListProps {
    emails: string[];
    sentCount: number;
    onDeleteEmail: (index: number | "all") => void;
    disabled?: boolean;
}

export function RecipientsList({ emails, sentCount, onDeleteEmail, disabled }: RecipientsListProps) {
    if (emails.length === 0) {
        return (
                ...
                <ScrollArea className="h-[500px] rounded-xl border border-muted-foreground/20 bg-muted/20">
                    <div className="p-3 space-y-2">
                        {emails.map((email, index) => (
                            <div
                                key={index}
                                className={`flex items-center gap-3 p-3 rounded-lg transition-all duration-200 ${index < sentCount
                            ...
                            >
                                ...
                                <span
                                    className={`text-sm flex-1 truncate ${index < sentCount
