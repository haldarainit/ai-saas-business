...
export default function Analytics({ products = [] }) {
  ...
  const analyticsData = useMemo(() => {
    ...
    // Shelf distribution
    const shelfMap = products.reduce((acc, product) => {
      const shelf = product.shelf || 'Unassigned';
      acc[shelf] = (acc[shelf] || 0) + 1;
      return acc;
    }, {});

    const shelfData = Object.entries(shelfMap).map(([name, count]) => ({
      name,
      count
    ...
    const summary = {
      ...
      expiringItems: products.filter(p => {
        ...
        const daysUntilExpiry = Math.ceil((new Date(p.expiryDate) - today) / (1000 * 60 * 60 * 24));
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Top profitable items</CardDescription>
        </CardHeader>
        <CardContent>
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={analyticsData.profitData}>
              <CartesianGrid strokeDasharray="3 3" />
              <XAxis dataKey="name" angle={-45} textAnchor="end" height={80} />
              <YAxis />
              <Tooltip content={<CustomTooltip />} />
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Products across shelves</CardDescription>
        </CardHeader>
        <CardContent>
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={analyticsData.shelfData} layout="horizontal">
              <CartesianGrid strokeDasharray="3 3" />
              <XAxis type="number" />
