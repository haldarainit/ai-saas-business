...
interface CSVUploadProps {
    onUploadSuccess?: (result: UploadResultData) => void;
}
...
export default function CSVUpload({ onUploadSuccess }: CSVUploadProps) {
    ...
    const handleFileUpload = async (file: File): Promise<void> => {
        ...
        try {
            ...
            toast({
                title: 'Upload Failed',
                description: errorMessage || 'Failed to upload CSV file.',
                variant: 'destructive',
            });
        } finally {
            setUploading(false);
            ...
            <CardContent className="space-y-4">
                ...
                {uploadResult && !uploadResult.error && (
                    <div className="space-y-3">
                        ...
                        {/* Error Details */}
                        {((uploadResult.results?.errors?.length ?? 0) > 0 || (uploadResult.validationErrors?.length ?? 0) > 0) && (
                            <div className="mt-4">
                        ...
                        {/* Duplicate Details */}
                        {(uploadResult.results?.duplicates?.length ?? 0) > 0 && (
                            <div className="mt-4">
