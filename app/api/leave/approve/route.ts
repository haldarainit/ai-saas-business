...
export async function POST(request: NextRequest): Promise<NextResponse> {
    try {
        ...
        const body: ApproveLeaveRequest = await request.json();
        ...
        if (action === 'approve') {
            ...
            if (employee) {
                ...
                if (leaveBalance && (leaveBalance[leaveBalanceKey] ?? 0) >= leave.days) {
                    leaveBalance[leaveBalanceKey] = (leaveBalance[leaveBalanceKey] ?? 0) - leave.days;
                    await employee.save();
                }
            }
