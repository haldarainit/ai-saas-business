...
import EmailTracking, { IEmailTracking } from "../../../lib/models/EmailTracking";
...
export async function GET(request: NextRequest): Promise<Response> {
    try {
        ...
        const query: AnalyticsQuery = { userId };
        if (campaignId) query.campaignId = campaignId;

        const [totals, recent] = await Promise.all([
            EmailTracking.aggregate<AggregatedTotals>([
                { $match: query },
                {
                    $group: {
                        _id: null,
        ...
        const t: AggregatedTotals = totals[0] || {
            _id: null,
            totalSent: 0,
            totalOpened: 0,
            totalClicked: 0,
        ...
        const response: AnalyticsResponse = {
            ...
            recent: recent as IEmailTracking[],
        };
