    ...
    try {
        ...
        if (!userProfile?.googleCalendar?.connected) {
            return NextResponse.json({
                success: true,
                connected: false,
                hasCredentials: !!(userProfile?.googleCalendar?.clientId && userProfile?.googleCalendar?.clientSecret),
        ...
        try {
            ...
            if (connectionStatus.connected) {
                return NextResponse.json({
                    success: true,
                    connected: true,
                    email: connectionStatus.email || userProfile.googleCalendar.connectedEmail,
...
export async function POST(request: NextRequest): Promise<NextResponse> {
    try {
        ...
        return NextResponse.json({
            success: true,
            message: "Google API credentials saved. You can now connect your calendar.",
            credentials: {
