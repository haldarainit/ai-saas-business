...
function resolveDateFromMessage(message) {
...
export async function POST(request) {
  try {
    ...
    try {
      ...
      gemini = geminiModule.default || geminiModule;
      
      if (!gemini || !gemini.generateAIResponse) {
        throw new Error("Gemini module not properly loaded");
      }
    } catch (importError) {
      console.error("Failed to import Gemini:", importError);
      throw new Error(`Failed to load AI service: ${importError.message}`);
    }
...
7. IMPORTANT: Always use the actual data from the attendance record when available - don't make up information!`;
    ...
    console.error("HR AI attendance error:", error);
