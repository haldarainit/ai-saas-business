...
interface EnhancedItem extends ParsedItem {
    id: string;
    needsReview: boolean;
}
...
export async function POST(request: NextRequest): Promise<NextResponse> {
    try {
        ...
        const inventoryType = (formData.get('inventoryType') as string) || 'trading';

        // Collect all files to process
        const filesToProcess: File[] = [];
        if (files && files.length > 0) {
            filesToProcess.push(...files.filter((f): f is File => f instanceof File));
        }
      ...
      "items": [
        {
          ...
          "gstPercentage": number,
          "gstAmount": number,
          "${inventoryType === 'manufacturing' ? 'costPerUnit' : 'costPrice'}": number,
          ${inventoryType !== 'manufacturing' ? '"sellingPrice": number,' : ''}
          "quantity": number,
          "totalCost": number,
...
{"error": "reason", "items": []}`;
        ...
        let allItems: EnhancedItem[] = [];
        let supplierInfo: SupplierInfo | null = null;
        ...
        for (const file of filesToProcess) {
            try {
                ...
                if (!textResponse || textResponse.startsWith('Error:')) {
                    ...
                    processedFiles.push({ name: file.name, status: 'error', error: textResponse || 'Unknown error' });
                    continue;
                }
                ...
                if (parsedData.items && Array.isArray(parsedData.items)) {
                    ...
                    const enhancedItems: EnhancedItem[] = parsedData.items.map((item, index) => {
                ...
                const err = fileError as Error & { status?: number };
                ...
                processedFiles.push({ name: file.name, status: 'error', error: err.message });
            }
        }
