...
interface GenerateActionPlanBody {
    strategy: Strategy;
    prompt?: string;
...
Guidelines:
- Make all content SPECIFIC to the ${strategy.channelType || "marketing"
...
- Recommend 3-4 tools that are most relevant to this strategy`;
        ...
        const aiResponse: string = await gemini.generateAIResponse(actionPlanPrompt);
        ...
        try {
            ...
            const actionPlan: ActionPlan = JSON.parse(jsonStr);
