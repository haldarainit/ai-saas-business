 ...
 *
 * Run with: node test-campaign-storage.ts
 */
...
async function testCampaignStorage() {
    ...
    try {
        ...
        if (loadedCampaign.csvData.data && loadedCampaign.csvData.data.length !== testCsvData.data.length) {
            throw new Error(
